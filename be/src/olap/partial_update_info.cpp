...
Status FlexibleReadPlan::read_columns_by_plan(
    ...
    for (const auto& [rowset_id, segment_row_mappings] : row_store_plan) {
        for (const auto& [segment_id, mappings] : segment_row_mappings) {
            ...
            auto st = doris::Tablet::fetch_value_through_row_column(rowset_iter->second,
                                                                    tablet_schema, segment_id, rids,
...
Status FlexibleReadPlan::fill_non_primary_key_columns_for_column_store(
    ...
    auto default_value_block = old_value_block.clone_empty();
    if (has_default_or_nullable || delete_sign_column_data != nullptr) {
        RETURN_IF_ERROR(Tablet::generate_default_value_block(tablet_schema, non_sort_key_cids,
                                                             info->default_values, old_value_block,
                                                             default_value_block));
    }
