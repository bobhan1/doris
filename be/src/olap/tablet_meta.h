...
// UT
    ...
    size_t tablet_local_common_index_size() const;
    size_t tablet_remote_common_index_size() const;
...
inline size_t TabletMeta::tablet_local_common_index_size() const {
    size_t total_size = 0;
    for (auto& rs : _rs_metas) {
        if (rs->is_local()) {
            total_size += rs->common_index_size();
        }
    }
    return total_size;
}

inline size_t TabletMeta::tablet_remote_common_index_size() const {
    size_t total_size = 0;
    for (auto& rs : _rs_metas) {
        if (!rs->is_local()) {
            total_size += rs->common_index_size();
