...
Status VerticalSegmentWriter::_append_block_with_flexible_partial_content(
    ...
    full_block = _tablet_schema->create_block();
    std::vector<uint32_t> sort_key_cids(_num_sort_key_columns);
    std::iota(sort_key_cids.begin(), sort_key_cids.end(), 0);
    for (auto cid : sort_key_cids) {
    ...
    std::vector<vectorized::IOlapColumnDataAccessor*> key_columns;
    RETURN_IF_ERROR(_olap_data_convertor->set_source_content_with_specifid_columns(
            &full_block, data.row_pos, data.num_rows, sort_key_cids));
    for (auto cid : sort_key_cids) {
        auto [status, column] = _olap_data_convertor->convert_column_data(cid);
        if (!status.ok()) {
            return status;
    ...
    bool has_sequence_col = _tablet_schema->has_sequence_col();
    if (has_sequence_col) {
        ...
        RETURN_IF_ERROR(_olap_data_convertor->set_source_content_with_specifid_column(
                full_block.get_by_position(seq_col_idx), data.row_pos, data.num_rows, seq_col_idx));
    ...
    auto* skip_bitmap_nullable_col_ptr = assert_cast<vectorized::ColumnNullable*,
    ...
    FlexiblePartialUpdateReadPlan read_plan;
    ...
    for (size_t block_pos = data.row_pos; block_pos < data.row_pos + data.num_rows; block_pos++) {
        ...
        size_t delta_pos = block_pos - data.row_pos;
        size_t segment_pos = segment_start_pos + delta_pos;
        const auto& skip_bitmap = skip_bitmaps->at(block_pos);

        std::string key = _full_encode_keys(key_columns, delta_pos);
        _maybe_invalid_row_cache(key);
        if (has_sequence_col && !skip_bitmap.contains(seq_col_unique_id)) {
            _encode_seq_column(seq_column, delta_pos, &key);
        }
        ...
        // save rowset shared ptr so this rowset wouldn't delete
        RowsetSharedPtr rowset;
        auto st = _tablet->lookup_row_key(key, has_sequence_col, specified_rowsets, &loc,
                                          _mow_context->max_version, segment_caches, &rowset);
        if (st.is<KEY_NOT_FOUND>()) {
            if (_opts.rowset_ctx->partial_update_info->is_strict_mode) {
                ++num_rows_filtered;
        ...
        //    the sequence columns, otherwise it may cause the merge-on-read based compaction
        //    policy to produce incorrect results
        if (have_delete_sign && !has_sequence_col) {
            has_default_or_nullable = true;
            use_default_or_null_flag.emplace_back(true);
        } else {
            // partial update should not contain invisible columns
    ...
    for (auto cid = _tablet_schema->num_columns() - _num_sort_key_columns;
        ...
        if (_opts.rowset_ctx->partial_update_info->is_flexible_partial_update()) {
            if (block->columns() != _tablet_schema->num_columns()) {
                return Status::InvalidArgument(
                        fmt::format("illegal flexible partial update block columns: {}, num key "
                                    "columns: {}, total "
                                    "schema columns: {}",
