        ...
        if (_seq_col_idx_in_block == -1) {
            ...
            auto finalize_rows = [&]() {
                if (row_with_delete_sign != nullptr) {
                    _finalize_one_row<is_final, false>(row_with_delete_sign, block_data,
                                                       row_pos_with_delete_sign);
                    row_with_delete_sign = nullptr;
                }
                if (row_without_delete_sign != nullptr) {
                    _finalize_one_row<is_final, false>(row_without_delete_sign, block_data,
            ...
            auto finalize_rows = [&]() {
                if (row_with_seq_col != nullptr) {
                    _finalize_one_row<is_final, false>(row_with_seq_col, block_data,
                                                       row_pos_with_seq);
                    row_with_seq_col = nullptr;
                }
                if (row_without_seq_col != nullptr) {
                    _finalize_one_row<is_final, false>(row_without_seq_col, block_data,
                                                       row_pos_without_seq);
