...
void MemTable::_aggregate_for_flexible_partial_update_without_seq_col(
    ...
    auto finalize_rows = [&]() {
        if (row_with_delete_sign != nullptr) {
            ...
            _finalize_one_row<is_final, false>(row_with_delete_sign, block_data, ++row_pos);
            row_with_delete_sign = nullptr;
        }
        if (row_without_delete_sign != nullptr) {
            temp_row_in_blocks.push_back(row_without_delete_sign);
            _finalize_one_row<is_final, false>(row_without_delete_sign, block_data, ++row_pos);
...
bool MemTable::need_flush() const {
    DBUG_EXECUTE_IF("MemTable::need_flush.force_flush", { return true; });
