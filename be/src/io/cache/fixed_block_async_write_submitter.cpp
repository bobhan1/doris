...
FixedBlockSubmitResult FixedBlockAsyncWriteSubmitter::try_submit(
    ...
    auto* service = request.cache->async_write_service();
    DORIS_CHECK(service != nullptr);
    if (!service->is_current_write_epoch(request.write_epoch)) {
        return FixedBlockSubmitResult::STALE_EPOCH;
    }

    ReadStatistics read_stats;
    CacheContext cache_context;
    cache_context.query_id = request.admission_ctx.query_id;
    cache_context.cache_type = request.admission_ctx.cache_type;
    cache_context.expiration_time = request.admission_ctx.expiration_time;
    cache_context.tablet_id = request.admission_ctx.tablet_id;
    cache_context.is_warmup = request.admission_ctx.is_warmup;
    ...
    if (!service->allocate_tracked_buffer(block_size, &tracked_buffer).ok()) {
