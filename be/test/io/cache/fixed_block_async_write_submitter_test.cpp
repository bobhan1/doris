...
protected:
    ...
    static void wait_for_writes(BlockFileCache* cache) {
        for (size_t attempt = 0;
             attempt < 500 && cache->async_write_service()->pending_count() != 0; ++attempt) {
...
TEST_F(FixedBlockAsyncWriteSubmitterTest, RejectsStaleAndExistingInflightOwnership) {
    ...
    cache->async_write_service()->invalidate_pending_writes();
    ...
    ASSERT_TRUE(cache->async_write_service()
                        ->allocate_tracked_buffer(BLOCK_SIZE, &existing_buffer)
                        .ok());
    auto existing_entry = std::make_shared<InflightWriteBufferEntry>(
            existing_buffer, 0, BLOCK_SIZE, MonotonicMicros(), existing_request.write_epoch);
