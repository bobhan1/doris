...
suite("test_cloud_mow_multi_seg_sc_merged_rows", "nonConcurrent") {
    if (!isCloudMode()) {
        return
    }

    def table1 = "test_cloud_mow_multi_seg_sc_merged_rows"
    ...
    setBeConfigTemporary(customBeConfig) {
        try {
            ...
            // inject cache miss in publish
            GetDebugPoint().enableDebugPointForAllBEs("CloudTxnDeleteBitmapCache::get_delete_bitmap.cache_miss")

            // block schema change process so that load will skip to calculate delete bitmap between segments
            ...
            loadMultiSegmentData(table1, 4096)
