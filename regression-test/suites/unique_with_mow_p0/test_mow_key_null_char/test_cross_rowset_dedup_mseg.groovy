...
// OPENSOURCE-272 Case 9: Multi-segment version of case 7.
// Old rowset has dirty key, new rowset has normal key + dirty key in DIFFERENT segments.
...
suite("test_cross_rowset_dedup_mseg", "nonConcurrent") {

    GetDebugPoint().clearDebugPointsForAllBEs()

    def mowTable = "test_cross_rs_dedup_mseg_mow"
    def morTable = "test_cross_rs_dedup_mseg_mor"
    ...
    // Dirty key SQL expression: 'ATE' padded with \0 to length 50
    def dirtyKeyExpr = "concat('ATE'" + ", char(0)" * 47 + ")"

    // Rowset 1 (old version): dirty key only, via SQL INSERT
    ...
    // dirty key deduped across rowsets
    ...
    // Row 2: dirty key (latest version, deduped from old rowset)
    assertEquals(50, mow_detail[1][1], "Dirty key length should be 50")
    assertEquals(18, mow_detail[1][3], "Dirty key value should be 18 (latest)")
    ...
    assertEquals(0, mow_dup.size(), "MOW should have no duplicate keys (multi-segment)")
