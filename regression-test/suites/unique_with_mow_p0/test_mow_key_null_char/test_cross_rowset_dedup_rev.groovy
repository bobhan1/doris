...
// OPENSOURCE-272 Case 8: Reverse of case 7.
// Old rowset has normal key, new rowset has both normal key and dirty key.
// The normal key across rowsets should be properly deduped,
// dirty data should not affect normal key dedup.

suite("test_cross_rowset_dedup_rev") {

    def mowTable = "test_cross_rowset_dedup_rev_mow"
    def morTable = "test_cross_rowset_dedup_rev_mor"
    ...
    // Build a dirty key: 'ATE' padded with \0 to length 50
    def dirtyKeyExpr = "concat('ATE'" + ", char(0)" * 47 + ")"

    // Rowset 1 (old version): normal key only
    ...
    // Expected: 2 rows (normal 'ATE' deduped + dirty 'ATE\0...\0')
    ...
    assertEquals(2, mow_count[0][0], "MOW should have 2 rows: normal key from old rowset should be deduped")
    assertEquals(2, mor_count[0][0], "MOR should have 2 rows")

    // Verify with hex to inspect actual key content
    ...
    // Row 1: normal key (latest version)
    ...
    assertEquals(18, mow_detail[1][3], "Second row value should be 18")
    ...
    assertEquals(0, mow_dup.size(), "MOW should have no duplicate keys: dirty data should not affect normal key dedup")
