...
- **REGEX 模式在 SQL 中的转义**：regex 字符串用单引号包裹，如果模式中需要单引号本身，需要 `''` 转义。反斜杠 `\` 的处理取决于 Doris SQL parser 的字符串转义规则：
  ...
  - 需要明确定义并文档化
- **模式→ table_id 解析**：需要先遍历系统所有表算 INCLUDE 集合，再遍历一次算 EXCLUDE 集合。但由于模式仅在 Job 创建时解析一次（不做定期刷新），这是一次性开销
...
| MatchedTables | `ods.order, ods.user, ods.payment` | **新增**：当前匹配的表名列表（用存储的 table_id 反查出来的当前表名） |
...
>
> **MatchedTables** 展示说明：
> - 基于持久化的 `table_id` 集合，在 SHOW 时通过 FE 元数据反查当前表名
> - 如果某个 table_id 对应的表已被 DROP，该 table_id 不出现在 MatchedTables 中
> - 如果某张表被 RENAME，MatchedTables 中展示的是**新表名**（因为 table_id 不变）
> - 这使用户可以清晰看到 Job 实际覆盖了哪些表
...
#### 3.1.4 模式 → table_id 集合的解析（FE 侧）

FE **仅在 Job 创建时**遍历所有 Database/Table 元数据，将匹配的表解析为 `table_id` 集合。后续运行时直接使用 `table_id` 进行过滤，不再重新匹配模式。
...
**存储策略：持久化 table_id 集合 + 原始 ON TABLES 表达式**

| 存储内容 | 用途 |
|----------|------|
| `tableIds`（table_id 集合） | **核心过滤依据**。持久化到 EditLog，运行时 O(1) 过滤，不受 RENAME 影响 |
| `tableFilterExpr`（原始 ON TABLES 表达式） | **仅用于展示**。SHOW WARM UP JOB 时展示原始创建意图 |

> **设计要点**：
> - ON TABLES 子句**仅在 Job 创建时生效一次**，之后所有过滤基于 table_id
> - 这意味着创建 Job 后新建的匹配表**不会自动纳入**，需要用户手动创建新 Job 或更新
> - 好处：实现简单、行为确定、RENAME 不影响过滤
...
public class CloudWarmUpJob {
    ...
    protected String tableFilterExpr = "";              // ON TABLES 子句的原始表达式，仅用于 SHOW 展示

    @SerializedName(value = "tableIds")
    protected Set<Long> tableIds = new HashSet<>();     // 持久化的 table_id 集合，过滤核心
...
| 决策点 | 决定 | 原因 |
|--------|------|------|
| 持久化内容 | table_id 集合 + 原始 ON TABLES 表达式 | table_id 是过滤依据，表达式仅用于展示 |
| 模式匹配时机 | 仅 Job 创建时匹配一次 | 行为确定、实现简单 |
| RENAME 行为 | 不受影响 | 基于 table_id 过滤 |
| 新建表 | 不自动纳入 | 需要创建新 Job |
...
public long createJob(WarmUpClusterCommand stmt) {
    ...
    if ("event_driven".equals(syncModeStr)) {
        ...
        CloudWarmUpJob job = new CloudWarmUpJob.Builder()
            ...
            .setTableFilterExpr(tableFilterExpr)    // 仅用于展示
            .setTableIds(tableIds)                  // 过滤核心
            .build();
        // ...
    }
}
...
**注意**：Job 类型保持 `CLUSTER`（因为基于集群语法创建），但内部持有 `tableIds` 标识是否为表级别过滤。
...
#### 3.2.5 getTabletReplicaInfos 无需修改
...
> **注意**：`has_table_filter` 参数区分两种语义：
> - `has_table_filter = false`：集群级别 Job（兼容旧版本语法，无 ON TABLES 子句）
> - `has_table_filter = true, table_ids 为空`：表级别 Job 但没有匹配到任何表（创建时应已被 FE 拒绝）
...
| BE 侧过滤器 | `order_tmp` 的 table_id 仍在 `EventDrivenJobFilter` 的 `table_ids` 中，但不会有新 rowset 触发 |
| Job 状态 | **不自动取消**。其他匹配的表继续正常预热 |

> Job 保持 RUNNING。SHOW WARM UP JOB 中 `MatchedTables` 通过 table_id 反查，已删除的表不会出现，用户可以看到实际覆盖的表数量减少。

### 5.2 新建表（CREATE TABLE）

**场景**：`ON TABLES (INCLUDE 'db1.order_*')`，用户新建了 `db1.order_2025`。

**行为**：新表**不会自动纳入**已有 Job 的预热范围。ON TABLES 子句仅在 Job 创建时生效一次，之后基于 table_id 集合过滤。

**解决方式**：用户需要创建新的 event-driven Job 来覆盖新表，或取消旧 Job 后重新创建（此时 ON TABLES 规则会重新匹配到新表）。

> 这是有意的设计选择：行为确定、可预测，不会因后台自动刷新引入非预期的预热流量。

### 5.3 表被重命名（ALTER TABLE RENAME）

**场景**：Job 创建时匹配到 `db1.order_2024`（table_id=1001），后被重命名为 `db1.archive_2024`。

**行为**：
- 过滤基于 table_id（1001），RENAME 不影响，预热**继续工作**
- SHOW WARM UP JOB 的 MatchedTables 展示**新表名** `db1.archive_2024`（因为用 table_id 反查当前名）
- TableFilter 仍显示原始 ON TABLES 表达式（供参考）
...
**解决方案**：
1. FE 定期检查 Source Cluster 的 BE 列表，对新增 BE 补发 `SET_JOB` RPC（包含 table_ids）
...
CloudWarmUpJob 通过 `EditLog` 持久化。新增字段 `tableFilterExpr` 和 `tableIds` 都有 `@SerializedName`。

FE 切主后：
1. 从 EditLog 恢复所有 job，包括 `tableIds` 和 `tableFilterExpr`
2. **无需重新解析模式**——table_id 集合已持久化
3. 向所有 Source BE 重新发送 `SET_JOB` 注册触发器，携带 `tableIds`
...
| `ON TABLES (INCLUDE 'nonexistent_db.*')` | 创建时校验，当前无匹配表则报错 |
...
每个表级别 job 额外存储：
- BE 侧 `EventDrivenJobFilter`：`std::optional<unordered_set<int64_t>>` — 集群级别为 `nullopt`（~1 byte overhead），表级别典型 5 张表 ≈ 80 bytes + optional overhead
- FE 侧 `tableIds`：`Set<Long>` — 同上
- 可忽略不计
...
@SerializedName(value = "tableIds")
protected Set<Long> tableIds = new HashSet<>();
```

**兼容性分析**：
| 场景 | 行为 | 安全 |
|------|------|------|
| **新 FE 读旧 EditLog**（旧 job 无新字段） | GSON 反序列化时缺少的字段使用 Java 字段初始值：`tableFilterExpr=""`, `tableIds=new HashSet<>()` → 等同于集群级别 Job（不过滤） | ✅ |
| **旧 FE 读新 EditLog**（新 job 含新字段） | GSON 遇到未知 JSON key 默认忽略，不会报错 | ✅ |
| **新 FE 读新 EditLog** | 正常反序列化 | ✅ |
...
public static class Builder {
    ...
    public Builder setTableIds(Set<Long> ids) { this.tableIds = ids; return this; }
}
```

**兼容性分析**：Builder 是创建时的辅助类，不涉及持久化。新增 setter 不影响已有代码的调用路径。旧代码不调用新 setter，新字段使用默认值（空串/空集合），等同集群级别 → **安全**。
...
| 变更点 | 持久化？ | 旧版本兼容 | 风险 |
|--------|---------|-----------|------|
| `CloudWarmUpJob` 新增 `tableIds` + `tableFilterExpr` | ✅ EditLog (GSON JSON) | GSON 忽略未知字段 / 缺失字段用默认值 | ✅ 无 |
...
| **EditLog 混合** | 旧 FE 写的 EditLog 无新字段 → 新 FE 反序列化后 `tableIds=[]`（集群级别）✅ | 新 FE 写的 EditLog 含新字段 → 旧 FE 反序列化时 GSON 忽略未知 key ✅ |
...
9. **新建表不自动纳入**：创建匹配模式的新表 → 验证不被预热（需新建 Job）
10. **删除表**：删除匹配表 → 验证 job 继续运行，其他表不受影响
11. **重命名表**：重命名后 → 验证该表仍被预热（基于 table_id），SHOW 中展示新名
...
| 5 | 过滤粒度 | DB + 表级别，不支持分区 |
| 6 | 模式匹配时机 | **仅 Job 创建时匹配一次**，后续基于 table_id 过滤 |
| 7 | 持久化内容 | table_id 集合（过滤核心）+ 原始 ON TABLES 表达式（仅展示） |
...
| 13 | RENAME | 不受影响（基于 table_id），SHOW 展示新表名 |
| 14 | 新建表 | 不自动纳入，需创建新 Job |
| 15 | DROP TABLE | Job 不自动取消，被删表自然不再触发 |
