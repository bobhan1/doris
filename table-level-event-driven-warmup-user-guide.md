...
`ON TABLES` 中的匹配规则**仅在 Job 创建时执行一次**。系统会在创建时扫描所有符合条件的表，将匹配到的表 ID 记录下来。之后的过滤基于表 ID 进行，不再重新匹配模式。

**这意味着**：
- 创建 Job 后**新建的表**不会自动纳入已有 Job 的预热范围，即使表名符合模式
- 如需覆盖新表，需要取消旧 Job 后重新创建（此时模式会重新匹配到新表），或为新表单独创建 Job
...
- `SHOW WARM UP JOB` 的 `MatchedTables` 中不再显示已删除的表

### 表被重命名（ALTER TABLE RENAME）

如果已匹配的某张表被重命名：
- **预热继续正常工作**（系统基于内部表 ID 过滤，RENAME 不影响表 ID）
- `SHOW WARM UP JOB` 的 `MatchedTables` 中会展示表的**新名称**
- `TableFilter` 仍显示创建时的原始规则
...
**不会**。模式只在 Job 创建时匹配一次。需要取消旧 Job 后重新创建，或为新表单独创建 Job。

### Q：表被 RENAME 后还会继续预热吗？

**会**。预热基于内部表 ID，RENAME 不改变表 ID，预热不受影响。
