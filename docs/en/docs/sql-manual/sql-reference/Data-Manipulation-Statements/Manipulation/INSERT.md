...
For Unique tables with merge-on-write enabled, the insert statement can also be used to perform partial column updates. To perform partial colums updates using the insert statement, the value of the session variable "enable_unique_key_partial_update" needs to be set to true (the default value of this variable is false, which means partial column updates using insert statements are not allowed by default). When performing partial column updates with the insert statement, the columns being inserted must include at least all the Key columns, and the specified columns to be updated. When performing partial column updates, if a row is inserted with values in key columns that don't exist in the original table, the columns not specified in the insert statement must either have default values or be nullable. These missing columns will be first attempted to be filled with default values. If the column does not have a default value, an attempt will be made to fill it with a null value. If the column cannot accept null values, the insert operation will fail.
