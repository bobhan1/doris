...
| --- | --- | --- | --- | --- |
| `COUNT` | segment footer / row count metadata | 文件尾部随机 footer 读 | segment open 后即可产出 | 12 bytes + `footer_length`；不读 ordinary data page |
| `MINMAX` / `MIX` | footer / column meta 中的 segment zone map | footer/meta 读 | 依赖 column meta；不依赖 page zone map indexed-column | footer 或 V3 meta slice / region |
...
`COUNT` 和 `MINMAX` 的读路径是 metadata-first，也基本是 metadata-only。它们对 cold read
的特点是：扫描行数可以很大，但文件 I/O 与行数没有线性关系，主要取决于 segment 数、
footer 是否命中 segment cache、V3 column meta 是否需要额外读取。
...
| 第二阶段 row store | hidden row-store column data page | 每个 segment 内 rowids 排序后单调推进；dense 复用 page，sparse 随机 | 依赖 rowid 分组、segment load、row-store column iterator | 每个触碰 page 是 `PagePointer.size`；single-row 也读整页 |
| 第二阶段 column store | 每个 lazy slot 的 data page / 子列 page | 每列独立按 sorted rowids 读；多列之间各自触碰 page | 依赖 lazy slot 列表和 rowids；复杂类型依赖 offsets / child | 列数越多，随机 page 数越容易放大 |
...
第二阶段的局部顺序性只在“同一 segment、同一 lazy column、排序后的 rowids”内成立。
跨 lazy columns、跨 segments、跨 backend RPC 的读取没有 page-level 顺序保证；宏观上常见
形态是多路小随机 page read。row store 能把多列合并成 hidden column 的 page read，但
如果 winners 分散在很多 data page 或 cache block 中，远端读取仍然会放大。
